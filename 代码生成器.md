...
选用JSON格式来存储，结构清晰、便于理解、对前端JavaScript非常友好

我们将元信息文件定义在 `meta.json`，之后放在制作工具的 `resources`目录下
...
比如**文件**输入路径、输出路径、文件类别（目录/文件）、生成类别（static/dynamic）等

3）记录数据模型信息

比如**参数**的名称、描述、类型、默认值等



在`maker.meta`包下新建`Meta`类，用于接收JSON字段。元信息`meta.json`存放在 resources 目录下。使用IDEA插件 GsonFormatPlus 生成java对象